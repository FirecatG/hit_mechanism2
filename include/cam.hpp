...
public:
    ...
    CamResults calculate() {
        CamResults results;
        // Generate varphi range (0-360 degrees)
        for (int deg = 0; deg <= 360; ++deg) {
            results.varphi_range.push_back(static_cast<double>(deg));
        }
        size_t num_points = results.varphi_range.size();

        // Initialize all vectors
        results.s_points.resize(num_points, 0.0);
        results.v_points.resize(num_points, 0.0);
        results.a_points.resize(num_points, 0.0);
        results.ds_dvarphi_points.resize(num_points, 0.0);
        ...
        for (size_t idx = 0; idx < num_points; ++idx) {
            double varphi = results.varphi_range[idx];
            double& s = results.s_points[idx];
            double& v = results.v_points[idx];
            double& a = results.a_points[idx];
            double& ds_dvarphi = results.ds_dvarphi_points[idx];
            double& alpha = results.alpha_points[idx];

            // Motion type calculations
            if (varphi <= phi_1) { // Rise phase
                if (rise_move_type == moveType::SIN_ACCELERATION) {
                    s = h * (varphi/phi_1 - std::sin(2*M_PI*varphi/phi_1)/(2*M_PI));
                    v = h*omega_1/phi_1 * (1 - std::cos(2*M_PI*varphi/phi_1));
                    a = 2*M_PI*h*omega_1*omega_1/(phi_1*phi_1) * std::sin(2*M_PI*varphi/phi_1);
                } 
                else if (rise_move_type == moveType::COS_ACCELERATION) {
                    s = h/2 * (1 - std::cos(M_PI*varphi/phi_1));
                    v = M_PI*h*omega_1/(2*phi_1) * std::sin(M_PI*varphi/phi_1);
                    a = M_PI*M_PI*h*omega_1*omega_1/(2*phi_1*phi_1) * std::cos(M_PI*varphi/phi_1);
                }
                alpha = alpha_1;
            }
            else if (varphi > phi_1 && varphi < phi_1 + phi_s1) { // Dwell 1
                s = h;
                v = 0.0;
                a = 0.0;
                alpha = alpha_1;
            }
            else if (varphi >= phi_1 + phi_s1 && varphi <= phi_1 + phi_s1 + phi_2) { // Return phase
                double T = varphi - (phi_1 + phi_s1);
            ...
            ds_dvarphi = v / omega_1;

            // Calculate r0 candidate
            double temp_r0 = e + 1.0;
            while (true) {
                double s0_temp = std::sqrt(temp_r0*temp_r0 - e*e);
                double compare = std::abs(ds_dvarphi - e)/(s0_temp + s) - std::tan(deg2rad(alpha));
        ...
        for (size_t idx = 0; idx < num_points; ++idx) {
            ...
            double s = results.s_points[idx];
            double ds_dvarphi = results.ds_dvarphi_points[idx];
            double dds_ddvarphi = results.a_points[idx] / omega_1;

            // Recalculate pressure angle
            double alpha_rad = std::atan(std::abs(ds_dvarphi - e)/(s0 + s));
            results.alpha_points[idx] = alpha_rad * 180.0 / M_PI;

            // Calculate coordinates
            double phi_rad = deg2rad(varphi);
            ...
            results.dx_points[idx] = -(s0 + s)*std::cos(phi_rad) - (ds_dvarphi - e)*std::sin(phi_rad);
            results.dy_points[idx] = -(s0 + s)*std::sin(phi_rad) + (ds_dvarphi - e)*std::cos(phi_rad);

            // Calculate curvature
            double ddx = -(2*ds_dvarphi - e)*std::cos(phi_rad) - (dds_ddvarphi - s0 - s)*std::sin(phi_rad);
            double ddy = (dds_ddvarphi - s0 - s)*std::cos(phi_rad) - (2*ds_dvarphi - e)*std::sin(phi_rad);
